...
/* Function prototypes */
void printout(int jid, pid_t currPID, int sig, int stopped);
...
void eval(char *cmdline) 
{
  ...
  if (cmd2 == NULL) //handle only 1 argument
    {
      if (!builtin_cmd(argv1))
	{
	  if (bg)
	    {
	      sigprocmask(SIG_BLOCK, &x, NULL); // block 
	      childPID = fork();
	      if (childPID == 0)
		{
		  setpgid(0, 0);
		  sigprocmask(SIG_UNBLOCK, &x, NULL); //child unblocks
		...
		{
		  setpgid(childPID, 0);
		  addjob(jobs, childPID, BG, cmdline); // parent addjob
		  sigprocmask(SIG_UNBLOCK, &x, NULL); // parent unblocks
	...
	exit(1);
      ...
      sigprocmask(SIG_BLOCK, &x, NULL); // Block to ensure
      if (fork() == 0)                  // good behavior
	{
	  dup2(fds[1], 1);

	  close(fds[0]);
	  close(fds[1]);
	  sigprocmask(SIG_UNBLOCK, &x, NULL); // Unblock to allow
	                                      // SIGCHLD
      ...
      if (fork() == 0)
	{
	  ...
	  close(fds[0]);
	  close(fds[1]);
	  sigprocmask(SIG_UNBLOCK, &x, NULL); // Also unblock here
...
int builtin_cmd(char **argv) 
{
  ...
  // Notice: 'quit' command was handled in eval - didn't feel the need
  // to handle it here, as I could just exit() in eval
...
void do_bg(int jid) 
{
  if (getjobjid(jobs, jid) -> state == ST)
    {
      getjobjid(jobs, jid) -> state = BG; // change state to background
      kill (getjobjid(jobs, jid)-> pid, SIGCONT); // then send CONT signal
...
void do_fg(int jid) 
{
  ...
  else if (getjobjid(jobs, jid) -> state == BG)
    {
      getjobjid(jobs, jid) -> state = FG;
      waitfg(getjobjid(jobs, jid)-> pid); 
    }
...
void waitfg(pid_t pid)
{
  ...
  /*
   *  The sigchld_handler function (via my implementation) simply 
   *  checks for a change in state, and if a FG child has exited,
   *  sigchld_handler simply changes the job's state to 'UNDEF', so that
   *  waitfg can update the jobs list by removing the job when waitfg
   *  finishes waiting for the foreground process to change state.
   */
  if (getjobpid(jobs, pid) -> state == UNDEF)
    deletejob(jobs, pid);
...
/*
 * Small helper funciton to allow signal handlers to print output
 * fashionably.
 */
void printout(int jid, pid_t currPID, int sig, int stopped)
{
  sio_puts("Job [");
  sio_putl(getjobpid(jobs, currPID) -> jid);
  sio_puts("] (");
  sio_putl(currPID);
  if (stopped)
...
void sigchld_handler(int sig) 
{
  ...
  if ((child = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
	...
	{
	  if (getjobpid(jobs, child) -> state == FG)
	    getjobpid(jobs, child) -> state = UNDEF; // change state (for waitfg)
	  else if(getjobpid(jobs, child) -> state == BG)
	    deletejob(jobs, child); // delete directly since nothing's waiting
	}
      else if (WIFSTOPPED(status)) // Child was stopped
	{
	  if (getjobpid(jobs, child)-> state != ST) // Only handle if the sigstp
	    sigtstp_handler(SIGTSTP);               // handler hasn't been called yet;
	}                                           // otherwise will loop indefinitely
      else if (WIFSIGNALED(status) && getjobpid(jobs, child) != NULL &&
	       getjobpid(jobs, child) -> state != UNDEF) // Interrupt from process
	sigint_handler(SIGINT);                          // outside the shell
...
void sigint_handler(int sig) 
{
  ...
  for (i = 0; i < MAXJOBS; i++)
    ...
    if (jobs[i].state == FG)
      {
	printout(getjobpid(jobs, currPID) -> jid, currPID, SIGINT, 0);
	getjobpid(jobs, currPID) -> state = UNDEF;
	kill(-currPID, sig);
      }
    }
  return;
...
void sigtstp_handler(int sig) 
{
  ...
  for (i = 0; i < MAXJOBS; i++)
    ...
    if (jobs[i].state == FG)
      {
	printout(getjobpid(jobs, currPID) -> jid, currPID, SIGTSTP, 1);
	getjobpid(jobs, currPID) -> state = ST;
	kill(-currPID, sig);
      }
    }
  return;

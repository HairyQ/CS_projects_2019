...
int main(int argc, char **argv) {
  ...
  server_dict = make_dictionary(COMPARE_CASE_INSENS, (free_proc_t)free_dictionary);
...
static void serve_friends(int fd, dictionary_t *query) {
  ...
  if (friends)
      ...
      for (; i < count; i++)
	{
	  ...
	  if (malloced)
	    free(realBody);
	  realBody = (char *)malloc(strlen(body));
	  malloced = 1;
...
static void serve_befriend(int fd, dictionary_t *query, int remove) {
  ...
  user = dictionary_get(query, "user");
  friend = dictionary_get(query, "friends");
	  ...
	  if (!remove)
	    {
	      ...
	      dictionary_t *newSet = make_dictionary(COMPARE_CASE_INSENS, NULL);
...
static void serve_introduce(int fd, dictionary_t *query) {
  ...
  //////////////////////////////////////////////////
  // Setting up / retrieving friend lists for this server
  //////////////////////////////////////////////////
  if (!dictionary_get(server_dict, user)) // Make sure we have entry for user
    {
      userFriends = make_dictionary(COMPARE_CASE_INSENS, NULL);
  ...
  if (!dictionary_get(server_dict, friend)) // Same for friend
    {
      friendsFriends = make_dictionary(COMPARE_CASE_INSENS, NULL);
      dictionary_set(friendsFriends, friend, NULL);
      dictionary_set(server_dict, user, friendsFriends);
    }
  else
    friendsFriends = dictionary_get(server_dict, friend);

  if (!socket_fd) // <- check to make sure we actually made a connection
    return;

...
// Declare the remote class and define the constructor and destructor.
//   (The body of the constructor and destructor will just have two
//   statements each for printing out that they are starting / ending
//   execution.  Use the same indentation and style as shown in the 
//   example output and the set and get functions below.)
...
observable::observable(int x) : value(x)
{
...
void observable::set (int new_value)
{
  ...
  if (new_value != get()) // TODO:  Fix this condition to match the comment above.
    {
...
class task_priority : public remote, public observable, public virtual value
{
public:
  task_priority(int);
  virtual ~task_priority();
  void set(int);
};

void task_priority::set(int x)
{
...
task_priority::task_priority(int x) : observable(x), remote(x), value(x)
{
